  ...
  it("bounds oversized legacy results and reports truncation without duplicating output in details", async () => {
    ...
    try {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run("oversized-session", "oversized-project", "/work/oversized", "2026-07-11T00:00:00.000Z", null, 1);
      db.prepare(`
        INSERT INTO messages (id, session_id, role, content, timestamp)
        VALUES (?, ?, ?, ?, ?)
      `).run("oversized-message", "oversized-session", "assistant", oversizedContent, "2026-07-11T00:01:00.000Z");
      registerSessionSearchTool(mockPi, dbManager);
