...
import { Box, type Component, visibleWidth } from "@earendil-works/pi-tui";
import { describe, it } from "vitest";
import { type SharedOutputView, createSharedToolResultRenderer } from "../../src/tools/shared-output-view.js";
import { memoryResultView, searchResultView, skillResultView } from "../../src/tools/tool-result-views.js";
...
function renderToolResult(toolResult: ReturnType<typeof result>, expanded: boolean, width = 120): string {
  return renderPlain(
    createSharedToolResultRenderer()(toolResult, { expanded, isPartial: false }, fakeTheme(), { isError: false }),
    width,
  );
}
...
function renderView(view: SharedOutputView, expanded: boolean, width = 120): string {
  return renderPlain(
    createSharedToolResultRenderer(() => view)(result(view.expandedText), { expanded, isPartial: false }, fakeTheme(), {
      isError: false,
    }),
    width,
  );
...
describe("shared tool-result view", () => {
  ...
  it("keeps the tool-card background after sanitizing expanded ANSI and CJK text", () => {
    ...
    const box = new Box(1, 1, (text: string) => `${backgroundOpen}${text}\x1b[49m`);
    box.addChild(
      createSharedToolResultRenderer()(result(expandedText), { expanded: true, isPartial: false }, ansiTheme, {
        isError: false,
      }),
    );
  ...
  it("uses Pi shell state for the card background and logical status for foreground", () => {
    ...
    for (const scenario of scenarios) {
      ...
      const rendered = renderPlain(
        renderer(toolResult, { expanded: false, isPartial: scenario.partial }, theme, { isError: scenario.isError }),
      );
      renderPlain(
        renderer(toolResult, { expanded: true, isPartial: scenario.partial }, theme, { isError: scenario.isError }),
      );
...
describe("tool-specific summaries", () => {
  ...
  it("summarizes successful failure memory from the real producer result shape", () => {
    ...
    assert.equal(view.summary, "Saved · target: failure · category: tool-quirk · 16 entries · 92% — 9271/10000 chars");
    assert.doesNotMatch(view.summary, /^failure:/);
    assert.equal(view.expandedText, fullText);
    assert.deepEqual(toolResult, before);

    const collapsed = renderPlain(
      createSharedToolResultRenderer(memoryResultView)(toolResult, { expanded: false, isPartial: false }, fakeTheme(), {
        isError: false,
      }),
    );
    assert.match(collapsed, /Saved · target: failure · category: tool-quirk · 16 entries · 92% — 9271\/10000 chars/);
