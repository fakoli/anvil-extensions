...
let reviewSettledSignal: Deferred;

function resetReviewSettledSignal(): void {
  reviewSettledSignal = deferred();
}
...
// Positive wait: poll until the fire-and-forget review chain has pushed the
// expected exec call. Vitest runs test files in parallel workers, so a fixed
// settle(5) can race the temp-file write on a loaded machine (upstream runs
// each file serially and never saw this).
async function waitForExecCall(targetExecCount: number, timeoutMs = 5000): Promise<void> {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    if (execCalls.length >= targetExecCount) return;
