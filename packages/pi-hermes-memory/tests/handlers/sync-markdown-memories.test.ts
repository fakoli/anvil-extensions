...
describe("memory sqlite sync + markdown backfill", () => {
  ...
  it("backfill command is idempotent across repeated runs", async () => {
    ...
    const userEntries = ["name: Chandra <!-- created=2026-05-08, last=2026-05-08 -->"];
  ...
  it("prunes Markdown orphans while preserving other targets and projects", async () => {
    ...
    assert.deepStrictEqual(
      getMemories(dbManager)
        .map((entry) => `${entry.project ?? "global"}:${entry.target}:${entry.content}`)
        .sort(),
      [
  ...
  it("waits for the canonical Markdown mutation before reading and reconciling", async () => {
    ...
    const coordinator = new AtomicLockCoordinator(
      path.join(path.dirname(path.dirname(identity)), ".pi-hermes-locks.sqlite"),
    );
  ...
  it("treats a missing canonical project file as empty despite a retained legacy backup", async () => {
    ...
    assert.strictEqual(fs.readFileSync(path.join(legacyProjectDir, "MEMORY.md"), "utf-8"), "retained legacy backup");
  });
  ...
  it("does not create a destination database after critical migration failure", async () => {
    ...
    try {
      await assert.rejects(
        migrateThenSyncMarkdownMemories(targetManager, legacyDir, targetDir, undefined, agentRoot, {
          moveFile: async () => {
  ...
  it("resolves a migrated file-symlink database at first I/O", { skip: process.platform === "win32" }, async () => {
    ...
    try {
      ...
      targetManager
        .getDb()
        .prepare("INSERT INTO extension_metadata (key, value) VALUES ('migrated-alias', 'kept')")
        .run();
      targetManager.close();
      ...
      try {
        assert.deepEqual(
          directManager.getDb().prepare("SELECT value FROM extension_metadata WHERE key = 'migrated-alias'").get(),
          { value: "kept" },
        );
      } finally {
        directManager.close();
