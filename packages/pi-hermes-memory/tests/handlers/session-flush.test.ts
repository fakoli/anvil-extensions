...
/** Minimal deferred for tests (repo lib targets ES2022, no Promise.withResolvers). */
interface Deferred {
...
function makeDirectDeps(result: DirectReviewResult | "throw"): {
...
describe("setupSessionFlush", () => {
  ...
  it("awaits the bounded flush before session_shutdown resolves", async () => {
    ...
    const { promise: execStarted, resolve: markExecStarted } = deferred();
    const { promise: releaseExec, resolve: release } = deferred();
  ...
  it("Flush uses pi.exec with correct args", async () => {
    ...
    assert.ok(message.includes("[ASSISTANT]"), "flush message should contain [ASSISTANT] prefix");
  ...
  it("forwards active cwd and model to flush subprocesses", async () => {
    ...
    const args = logicalChildArgs(mockPi.execCalls[0]);
    assert.deepStrictEqual(args.slice(0, 4), ["-p", "--no-session", "--model", "local-llama/local-9b"]);
  });
...
describe("direct transport", () => {
  ...
  it("session_before_compact uses direct transport without subprocess when direct returns ok", async () => {
    ...
    setupSessionFlush(mockPi.pi, mockStore, null, config, null, null, makeDirectDeps({ ok: true, appliedCount: 2 }));
  ...
  it("routes project facts to project target when a project store is available", async () => {
    const config = defaultConfig({ reviewTransport: "direct" });
    const projectStore = { getMemoryEntries: () => ["existing project fact"] } as unknown as Parameters<
      typeof setupSessionFlush
    >[2];
    setupSessionFlush(
      mockPi.pi,
      mockStore,
  ...
  it("falls back to subprocess when direct throws without propagating on compact", async () => {
    const config = defaultConfig({ reviewTransport: "direct" });
    setupSessionFlush(mockPi.pi, mockStore, null, config, null, null, makeDirectDeps("throw"));
  ...
  it("skips direct transport when reviewTransport is subprocess", async () => {
    ...
    setupSessionFlush(mockPi.pi, mockStore, null, config, null, null, makeDirectDeps({ ok: true, appliedCount: 99 }));
