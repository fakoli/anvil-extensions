...
describe("buildPromptContext", () => {
  ...
  it("appends standing instructions after the policy in policy-only mode", async () => {
    const result = await buildPromptContext({ memoryMode: "policy-only" }, store, projectStore, "demo", standing);
  ...
  it("puts standing instructions last in legacy-inject mode", async () => {
    const result = await buildPromptContext({ memoryMode: "legacy-inject" }, store, projectStore, "demo", standing);
