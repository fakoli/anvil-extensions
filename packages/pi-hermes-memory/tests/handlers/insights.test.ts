...
async function invoke(handler: Function, notifyCalls: { message: string; severity: string }[]): Promise<string> {
  await handler(
    {},
    {
      ui: {
...
describe("registerInsightsCommand", () => {
  ...
  it("shows MEMORY section with numbered entries", async () => {
    const { handler, notifyCalls } = await setupCommand(["first entry", "second entry"], []);
  ...
  it('shows "(empty)" when no entries exist', async () => {
    ...
    assert.ok(emptyMatches && emptyMatches.length === 2, "Expected (empty) in both MEMORY and USER sections");
  });
  ...
  it("entry numbering is sequential", async () => {
    const { handler, notifyCalls } = await setupCommand(["alpha", "beta", "gamma"], []);
    const output = await invoke(handler, notifyCalls);
