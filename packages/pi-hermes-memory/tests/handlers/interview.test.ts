...
let handlers: Record<string, Array<{ fn: Function; desc: string }>>;
...
describe("registerInterviewCommand", () => {
  ...
  it("shows notification when USER.md already has entries", async () => {
    ...
    assert.ok(notifyCalls[0].msg.includes("2 profile entries"), "notification should mention entry count");
    assert.ok(notifyCalls[0].msg.includes("Chandrateja"), "notification should show existing entry preview");
  });
