...
describe("SkillsManagerModal", () => {
  ...
  it("stops rendering updates after close during async actions", async () => {
    const harness = createModalHarness();
    // Object-holder: TS narrows a `let ... = null` whose only writes happen in
    // a closure to `never`, so keep the resolver behind a mutable property.
    const pendingMove: { resolve: ((result: SkillBatchActionResult) => void) | null } = { resolve: null };
    let closeCount = 0;
    ...
    pendingMove.resolve?.({ skills: SAMPLE_SKILLS, summaryLines: ["moved"] });
    await nextTick();
