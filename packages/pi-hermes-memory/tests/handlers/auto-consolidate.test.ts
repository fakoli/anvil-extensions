...
function createDirectCtx(): Pick<ExtensionContext, "model" | "modelRegistry"> {
  return { model: {}, modelRegistry: {} } as unknown as Pick<ExtensionContext, "model" | "modelRegistry">;
}

function makeDirectDeps(result: { ok: boolean; appliedCount: number } | "throw"): {
...
describe("triggerConsolidation", () => {
  ...
  it("clears a failed release before the next consolidation", async () => {
    ...
    prototype.deleteOwnedLock = function (key: string, token: string) {
      deleteAttempts++;
  ...
  it("defers a duplicate subprocess while the same target is consolidating", async () => {
    ...
    await withLockWait("0", async () => {
      ...
      for (const release of releaseExecs) release();
  ...
  it("allows the same project target to consolidate concurrently in distinct stores", async () => {
    ...
    const stores = ["project-a", "project-b"].map(
      (name) =>
        new MemoryStore({
  ...
  it("retries once without overrides when the override subprocess fails for model resolution reasons", async () => {
    ...
    assert.strictEqual(
      typeof retryArgs[retryArgs.length - 1],
      "string",
      "fallback retry should keep prompt as final arg",
    );
  });
...
describe("registerConsolidateCommand", () => {
  ...
  it("includes project memory when a project store is available", async () => {
    ...
    assert.ok(
      notifications.some((message) => message.includes("Starting memory consolidation")),
      "should show an initial progress notification",
    );
    assert.ok(
      notifications.some((message) => message.includes("⏳ Consolidating memory")),
...
describe("MemoryStore auto-consolidation integration", () => {
  ...
  it("add() distinguishes a consolidation that ran but freed nothing", async () => {
    ...
    assert.ok(result.error!.includes("Auto-consolidation ran but did not free enough space."), result.error);
    assert.ok(
...
describe("triggerConsolidation — raw entry round-trip", () => {
  it("prefers getRawEntriesForSync so metadata comments reach the prompt", async () => {
    const rawStore = {
      ...mockStore,
      getRawEntriesForSync: (target: string) =>
        target === "memory"
          ? ["raw entry <!-- created=2026-01-01, last=2026-09-09 -->"]
          : target === "user"
            ? ["raw user fact"]
            : ["raw failure lesson"],
    } as any;
    const localPi = createMockPi();
    await triggerConsolidation(localPi, rawStore, "memory");

    const args = logicalChildArgs(execCalls[execCalls.length - 1]);
    const prompt = args[args.length - 1];
    assert.ok(
      prompt.includes("created=2026-01-01"),
      "prompt should include the raw metadata comment, not a stripped entry",
    );
  });

  it("falls back to public getters when the store lacks the raw variant", async () => {
    const legacyStore = { ...mockStore }; // no getRawEntriesForSync
    const localPi = createMockPi();
    await triggerConsolidation(localPi, legacyStore, "memory");

    const args = logicalChildArgs(execCalls[execCalls.length - 1]);
    const prompt = args[args.length - 1];
    assert.ok(prompt.includes("old entry 1"), "legacy stores keep working");
  });
});
