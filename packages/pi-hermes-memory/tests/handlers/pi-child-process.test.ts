...
const OWN_EXTENSION_PATH = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../src/index.ts");
...
describe("inheritedExtensionArgs", () => {
  it("captures explicit -e and --extension parent args", () => {
    assert.deepStrictEqual(inheritedExtensionArgs(["-e", "src/index.ts", "--extension", "/tmp/other.ts"]), [
      "-e",
      "src/index.ts",
      "--extension",
      "/tmp/other.ts",
  ...
  it("captures --extension=... parent args", () => {
    assert.deepStrictEqual(inheritedExtensionArgs(["--extension=src/index.ts"]), ["--extension=src/index.ts"]);
  });
});
...
describe("buildChildPiPromptArgs", () => {
  it("uses --no-extensions and only passes hermes-memory extension", () => {
    assert.deepStrictEqual(buildChildPiPromptArgs("hello", {}, []), ["-p", "--no-session", ...EXT_ARGS, "hello"]);
  });
  ...
  it("inherits the active provider/model when no override is configured", () => {
    assert.deepStrictEqual(buildChildPiPromptArgs("hello", {}, [], { provider: "local-llama", id: "local-9b" }), [
      "-p",
      "--no-session",
      "--model",
  ...
  it("allows thinking overrides without a model override", () => {
    assert.deepStrictEqual(buildChildPiPromptArgs("hello", { llmThinkingOverride: "low" }, []), [
      "-p",
      "--no-session",
      "--thinking",
      "low",
  ...
  it("ignores missing inherited extension paths", () => {
    assert.deepStrictEqual(buildChildPiPromptArgs("hello", {}, ["-e", "src/index.ts"]), [
      "-p",
      "--no-session",
      ...EXT_ARGS,
      "hello",
  ...
  it("dedupes and forwards existing configured extension paths, dropping non-existent ones (npm 0.9.6 baseline, pre-#189)", async () => {
...
describe("resolveChildPiInvocation", () => {
  it("keeps non-Windows child pi invocations unchanged", () => {
    ...
    assert.deepStrictEqual(resolveChildPiInvocation(args, { platform: "linux" }), { command: "pi", args });
  });
  ...
  it("resolves a standard Windows command shim to its adjacent cli.js", async () => {
    ...
    const cliPath = path.join(binDirectory, "node_modules", "@earendil-works", "pi-coding-agent", "dist", "cli.js");
    const args = ["-p", "--no-session", "hello"];
...
describe("execChildPrompt", () => {
  ...
  it("hard-kills a child that ignores graceful timeout termination", async () => {
    ...
    const result = await new Promise<{ code: number | null; stderr: string }>((resolve) => {
      const child = spawn(
        process.execPath,
        [watchdogPath, "100", "-", process.execPath, "-e", "process.on('SIGTERM',()=>{});setInterval(()=>{},1000)"],
  ...
  it("removes the temporary prompt file when child execution throws", async () => {
    ...
    await assert.rejects(execChildPrompt(pi as any, "secret failure prompt", {}, { timeoutMs: 30000 }), /child failed/);
      ...
      await execChildPrompt(
        {
          exec: async (_cmd: string, args: string[]) => {
            calls.push(args);
            return calls.length === 1 ? { code: 1, stderr: "model not found" } : { code: 0, stdout: "ok" };
          },
        } as any,
        "private review",
        {
