...
describe("git worktree project identity (#120)", () => {
  ...
  it("scopes a subdirectory of the repository to the repository", () => {
    ...
    try {
      assert.strictEqual(detectProject("projects-memory", path.join(repo, "packages", "api")).name, "my-project");
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("keeps using an existing cwd-basename store instead of orphaning it", () => {
    ...
    try {
      ...
      assert.strictEqual(detectProject("projects-memory", path.join(repo, "packages", "api")).name, "api");
    } finally {
