...
describe("MemoryStore", () => {
  ...
  describe("replace()", () => {
    ...
    it("replaces identical failure text across project scopes while preserving each scope", async () => {
      ...
      assert.deepEqual(entries.map((entry) => entry.match(/project64=([A-Za-z0-9_-]+)/)?.[1]).sort(), [
  ...
  describe("remove()", () => {
    ...
    it("accepts a pasted memory_search line for normal memories", async () => {
      ...
      const result = await store.remove(
        "memory",
        `🧠 scope=global [target=memory] ${TEST_MARKER} prefers pnpm over npm\n   Created: 2026-05-27 | Last used: 2026-05-27`,
      );
      await settle();
  ...
  describe("atomic writes", () => {
    ...
    it(
      "mutates a file-symlinked Markdown target without replacing the link",
    ...
    it(
      "creates a dangling relative Markdown symlink target and preserves the link",
      { skip: process.platform === "win32" },
      async () => {
  ...
  describe("applyMutationPlan()", () => {
    ...
    it("rejects invalid plans before publishing any draft", async () => {
      ...
      const cases = [
        ...
        {
          operations: [{ action: "replace", oldText: "shared", content: `${TEST_MARKER} replacement` }],
          error: /Multiple entries matched 'shared'/,
        },
      ] as const;
    ...
    it("preserves replace metadata and matches scoped failure copies", async () => {
      ...
      assert.deepEqual(
        entries.map((entry: any) => entry.text),
        [`${TEST_MARKER} replaced`, `${TEST_MARKER} replaced`],
      );
      assert.deepEqual(
        entries.map((entry: any) => entry.created),
        [created, created],
      );
      assert.deepEqual(
        entries.map((entry: any) => entry.project),
        ["project-a", "project-b"],
      );
    });
    ...
    it("formats failure adds and deduplicates them within project scope", async () => {
      ...
      const entries = (store as any).failureEntries.map((entry: string) => (store as any).decodeEntry(entry));
      assert.deepEqual(
        entries.map((entry: any) => ({ text: entry.text, project: entry.project })),
        [
          {
    ...
    it("requires a strictly smaller final plan", async () => {
      ...
      for (const content of [`${TEST_MARKER} same form`, `${TEST_MARKER} this replacement is larger`]) {
        const result = await store.applyMutationPlan("memory", [{ action: "replace", oldText: "same size", content }], {
          requireShrink: true,
        });
        assert.equal(result.success, false);
  ...
  describe("external file changes", () => {
    ...
    it("rolls back a published mutation without copying through the temporary link", async () => {
      ...
      await assert.rejects(store.add("memory", `${TEST_MARKER} failed local add`), /injected post-publish failure/);
      const raw = await readRaw(memoryPath);
    ...
    it("does not commit a failed add during a later mutation", async () => {
      ...
      await assert.rejects(store.add("memory", `${TEST_MARKER} failed add`), /injected add save failure/);
    ...
    it("does not commit a failed removal during a later mutation", async () => {
      ...
      await assert.rejects(store.remove("memory", `${TEST_MARKER} retained entry`), /injected remove save failure/);
      const result = await store.add("memory", `${TEST_MARKER} later add`);
    ...
    it("prunes expired recovery files but retains recently active ones", async () => {
      ...
      const retiredFiles = (await fs.readdir(MEMORY_DIR)).filter((name) => name.startsWith(`.${MEMORY_FILE}.retired-`));
    ...
    it("ignores generated-looking recovery symlinks during pruning", async (t) => {
      ...
      assert.equal(
        retiredContents.some((content) => content.includes("outside sensitive content")),
        false,
      );
    });
    ...
    it("does not retain an active recovery when the upcoming snapshot consumes the byte budget", async () => {
      ...
      const activeStillExists = await fs.stat(recoveryPath).then(
        () => true,
        () => false,
      );
    ...
    it("bounds active recovery snapshots by count and bytes while keeping the newest", async () => {
      ...
      const recoveryPathFor = (
        pathStore as unknown as { recoveryPathFor(filePath: string): string }
      ).recoveryPathFor.bind(pathStore);
    ...
    it("bounds retired recovery snapshots by age, count, and bytes", async () => {
      ...
      const retiredStats = await Promise.all(retiredFiles.map((name) => fs.stat(path.join(MEMORY_DIR, name))));
    ...
    it("bounds generated conflict artifacts without following lookalike symlinks", async () => {
      ...
      const symlinkPath = path.join(MEMORY_DIR, `.${MEMORY_FILE}.conflict-local-${Date.now()}-${randomUUID()}`);
      ...
      const stalePath = path.join(MEMORY_DIR, `.${MEMORY_FILE}.conflict-local-${Date.now()}-${randomUUID()}`);
    ...
    it("serializes concurrent mutations of the same canonical target", async () => {
      ...
      const firstCanSave = new Promise<void>((resolve) => {
        releaseFirst = resolve;
      });
      let firstEntered!: () => void;
      const firstEnteredSave = new Promise<void>((resolve) => {
    ...
    it("allows different targets to mutate concurrently", async () => {
      ...
      const memoryCanSave = new Promise<void>((resolve) => {
        releaseMemory = resolve;
      });
      let memoryEntered!: () => void;
      const memoryEnteredSave = new Promise<void>((resolve) => {
    ...
    it("reloads truncated disk before add and reports disk-backed usage", async () => {
      ...
      assert.equal(
        await readRaw(memoryPath),
        `${TEST_MARKER} after-truncate <!-- created=${new Date().toISOString().split("T")[0]}, last=${new Date().toISOString().split("T")[0]} -->`,
      );
    });
...
describe("MemoryStore — echoed metadata comment strip", () => {
  it("adding content that carries a stale trailing comment does not double it", async () => {
    const store = new MemoryStore(makeConfig());
    await store.loadFromDisk();
    await store.add(
      "memory",
      "fact about routers <!-- created=2025-01-01, last=2025-09-09 -->",
    );
    const raw = store.getRawEntriesForSync("memory")[0];
    const commentCount = (raw.match(/created=/g) ?? []).length;
    assert.strictEqual(commentCount, 1, "exactly one metadata comment, freshly stamped");
    assert.match(raw, /fact about routers/);
  });
});
