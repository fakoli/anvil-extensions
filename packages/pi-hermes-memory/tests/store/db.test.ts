...
describe("DatabaseManager", () => {
  ...
  describe("schema", () => {
    ...
    it("rebuilds existing unicode61 FTS tables and preserves indexed data", () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `).run("cjk-message", "cjk-session", "assistant", "设备清单包含 NAS", "2026-05-03T00:01:00Z");
      db.prepare(`
        INSERT INTO memories (project, target, content, created, last_referenced)
        VALUES (?, ?, ?, ?, ?)
      `).run("test-project", "memory", "设备清单包含 NAS", "2026-05-03", "2026-05-03");
      ...
      assert.ok(migrated.prepare("SELECT rowid FROM message_fts WHERE message_fts MATCH ?").all("设备清单").length > 0);
      assert.ok(migrated.prepare("SELECT rowid FROM memory_fts WHERE memory_fts MATCH ?").all("设备清单").length > 0);
    });
    ...
    it("should migrate legacy target CHECK constraint to allow failure entries", () => {
      ...
      assert.deepStrictEqual(indexes.map((row) => row.name).sort(), [
  ...
  describe("corruption recovery", () => {
    it("waits for a recovery owner and reuses the healthy database it leaves behind", () => {
      ...
      dbManager = new DatabaseManager(tmpDir, {
        recoveryLockWaitMs: 1000,
        recoveryLockPollMs: 10,
        recoveryLockStaleMs: 10_000,
      });
    ...
    it("serializes recovery through symlinked database aliases", { skip: process.platform === "win32" }, () => {
      ...
      const coordinator = new AtomicLockCoordinator(
        path.join(path.dirname(canonicalDbPath), ".pi-hermes-locks.sqlite"),
      );
      ...
      try {
        assert.throws(() => aliasManager.getDb(), /SQLite recovery already in progress/);
      } finally {
        aliasManager.close();
        lease.release();
      }
    ...
    it(
      "repairs a file-symlinked database target without replacing the link",
      ...
      () => {
        ...
        try {
          ...
          assert.deepEqual(directDb.prepare("SELECT value FROM extension_metadata WHERE key = 'alias-write'").get(), {
            value: "kept",
          });
        } finally {
          directManager.close();
    ...
    it(
      "creates and repairs a dangling absolute database symlink through its target",
      { skip: process.platform === "win32" },
      () => {
        dbManager.close();
        const realDir = path.join(tmpDir, "real");
        ...
        aliasManager
          .getDb()
          .prepare("INSERT INTO extension_metadata (key, value) VALUES ('before-corruption', 'kept')")
          .run();
        aliasManager.close();
        fs.writeFileSync(realDbPath, "not a sqlite database");
    ...
    it("cleans abandoned rebuild files and caps corrupt backup sets", () => {
      ...
      assert.strictEqual(
        names.some((name) => name.startsWith("sessions.db.rebuild-")),
        false,
      );
    ...
    it("counts a failed post-recovery open before clearing the circuit", () => {
      ...
      const state = JSON.parse(fs.readFileSync(path.join(tmpDir, "sessions.db.recovery-state.json"), "utf-8"));
