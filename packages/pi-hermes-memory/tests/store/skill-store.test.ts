...
describe("SkillStore", () => {
  ...
  describe("patch()", () => {
    ...
    it("coerces JSON string arrays into ordered Procedure steps", async () => {
      ...
      const result = await store.patch(created.skillId!, "Procedure", JSON.stringify(["First step", "Second step"]));
      assert.ok(result.success, `patch failed: ${result.error}`);
    ...
    it("rejects patch content that injects section headers", async () => {
      ...
      const result = await store.patch(created.skillId!, "Procedure", "1. New\n\n## Pitfalls\n- Injected duplicate");
      assert.equal(result.success, false);
