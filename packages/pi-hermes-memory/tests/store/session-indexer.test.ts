...
import { type ParsedSession, parseSessionFile } from "../../src/store/session-parser.js";
...
describe("session-indexer", () => {
  ...
  describe("indexSession", () => {
    it("should index a session and its messages", () => {
      ...
      const messages = db.prepare("SELECT * FROM messages WHERE session_id = ?").all("session-1") as Record<
        string,
        unknown
      >[];
    ...
    it("should cap oversized message content while retaining both ends", () => {
      ...
      const stored = dbManager
        .getDb()
        .prepare("SELECT content FROM messages WHERE id = ?")
        .get("oversized-message") as { content: string };
    ...
    it("should append missing messages for an already-indexed resumed session", () => {
      ...
      const dbSession = dbManager
        .getDb()
        .prepare("SELECT message_count FROM sessions WHERE id = ?")
        .get("session-1") as { message_count: number };
  ...
  describe("current session indexing helpers", () => {
    ...
    it("parseSessionManagerSnapshot converts current session entries into ParsedSession", () => {
      const snapshot = createSessionManagerSnapshot([
        ...
        },
        {
          ...
          message: {
            role: "toolResult",
            content: [{ type: "text", text: "tool output is not indexed by current schema" }],
          },
        },
      ]);
    ...
    it("parseSessionManagerSnapshot skips embedded tool result content", () => {
      const parsed = parseSessionManagerSnapshot(
        createSessionManagerSnapshot([
          {
            type: "message",
            id: "entry-1",
            timestamp: "2026-05-03T00:01:00Z",
    ...
    it("indexLiveSession prefers the persisted JSONL file when available", () => {
      ...
      assert.strictEqual(result.messagesIndexed, 1);
      const indexed = dbManager.getDb().prepare("SELECT id, cwd FROM sessions WHERE id = ?").get("file-session-1") as {
        id: string;
        cwd: string;
      };
  ...
  describe("backfill metadata helpers", () => {
    ...
    it("needsBackfill is true when file metadata changes even with a recent timestamp", () => {
      ...
      fs.appendFileSync(
        path.join(sessionsDir, "project-a", "s1.jsonl"),
        "\n" +
          JSON.stringify({
            type: "message",
            id: "s1-m2",
    ...
    it("touchBackfillTimestamp upserts the metadata row", () => {
      ...
      const row = dbManager
        ...
        .get(LAST_SESSION_BACKFILL_KEY) as { value: string };
    ...
    it("upsertSessionFileMetadata keeps stored metadata in sync after a session file is appended", () => {
      ...
      assert.ok(session, "session file should parse");
      indexSession(dbManager, session);
