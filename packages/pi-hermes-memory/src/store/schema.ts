...
export const SCHEMA_SQL = `
  ...
  -- The UPDATE trigger skips rebuilds when content is unchanged: metadata-only
  -- updates (last_referenced bumps, category merges) previously rebuilt the
  -- FTS5 index for the row on every touch. DROP+CREATE upgrades existing
  -- databases in place (CREATE IF NOT EXISTS alone would keep the old body).
  -- Wrapped in a transaction below (SCHEMA_SQL runs via exec): DDL is
  -- transactional in SQLite, and concurrent processes either see old or new,
  -- never a dropped-trigger gap.
  BEGIN IMMEDIATE;
