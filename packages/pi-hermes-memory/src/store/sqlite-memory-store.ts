...
/** One- and two-character ASCII terms (e.g. "c1", "go", "RT") also fail
 *  trigram matching: FTS5's trigram tokenizer only indexes 3+ character
 *  sequences, so such queries can never hit the index. They need the same
 *  scoped literal fallback as short CJK terms. */
function isShortAsciiLiteralQuery(query: string): boolean {
  const trimmed = query.trim();
  return trimmed.length > 0 && trimmed.length <= 2 && /^[A-Za-z0-9]+$/.test(trimmed);
}
...
export function syncMemoryEntry(dbManager: DatabaseManager, input: SqliteMemorySyncInput): SqliteMemorySyncResult {
  ...
  // Skip the UPDATE entirely when nothing changed: the memories_au trigger
  // fires on every UPDATE, and rewriting identical metadata would rebuild the
  // FTS5 index for the row for no benefit (churn on every touch).
  const unchanged =
    updatedCategory === existing.category &&
    updatedFailureReason === existing.failure_reason &&
    updatedToolState === existing.tool_state &&
    updatedCorrectedTo === existing.corrected_to &&
    updatedCreated === existing.created &&
    updatedLastReferenced === existing.last_referenced;
  if (unchanged) {
    // DB already holds these values; skip the write (and the FTS trigger)
    // and return the stored row, same shape as the update path below.
    return {
      action: "existing",
...
export function reconcileMarkdownFailureScopes(
  ...
  for (const project of projects) {
    const result = reconcileMarkdownMemoryScope(dbManager, entriesByProject.get(project) ?? [], "failure", project);
...
export function searchMemories(
  ...
  if (isShortCjkLiteralQuery(query) || isShortAsciiLiteralQuery(query)) {
