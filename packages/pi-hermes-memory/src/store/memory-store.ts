...
export class MemoryStore {
  ...
  private async addWithConsolidation(
    ...
    const result = await this.runTargetMutation(target, (markMutation) =>
        ...
        if (operation.action === "add") {
          ...
          if (
            plannedEntries.some((entry) => {
              const decoded = this.decodeEntry(entry);
              return (
                decoded.text === normalizedContent && (target !== "failure" || decoded.project === normalizedProject)
              );
            })
          ) {
  ...
  async replace(target: "memory" | "user" | "failure", oldText: string, newContent: string): Promise<MemoryResult> {
    return this.runTargetMutation(target, (markMutation) =>
  ...
  async remove(target: "memory" | "user" | "failure", oldText: string): Promise<MemoryResult> {
    return this.runTargetMutation(target, (markMutation) => this.removeUnlocked(target, oldText, markMutation));
  }
  ...
  private encodeEntry(text: string, created: string, lastReferenced: string, project?: string): string {
    const projectMetadata = project?.trim()
      ...
      : "";
    // Drop a trailing metadata comment echoed from a raw entry (consolidation
    // children now see raw entries and may copy the comment verbatim into
    // new_content); a stale comment here would survive decodeEntry as visible
    // text and produce double comments in the store.
    const stripped = text.replace(/\s*<!--\s*created=[^>]*-->\s*$/u, "").trimEnd();
    return `${stripped} <!-- created=${created}, last=${lastReferenced}${projectMetadata} -->`;
  }
  ...
  private validateWholeEntryReplacement(entries: string[], oldText: string, newContent: string): string | undefined {
    for (const entry of entries) {
      ...
      const missingLines = entryLines.filter((line) => !line.includes(oldText) && !newContent.includes(line));
