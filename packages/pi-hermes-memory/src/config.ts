...
export function loadConfig(configPath = DEFAULT_CONFIG_PATH): MemoryConfig {
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (typeof parsed.standingInstructionsEnabled === "boolean")
        config.standingInstructionsEnabled = parsed.standingInstructionsEnabled;
      ...
      if (isStringArray(parsed.childExtensionPaths)) {
        const childExtensionPaths = [
          ...new Set<string>((parsed.childExtensionPaths as string[]).map((item) => item.trim()).filter(Boolean)),
        ];
