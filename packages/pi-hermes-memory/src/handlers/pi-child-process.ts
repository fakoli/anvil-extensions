...
export function resolveChildPiModel(model: { provider?: string; id?: string } | undefined): ChildPiModel | undefined {
...
const CHILD_PROCESS_WATCHDOG_PATH = fileURLToPath(new URL("./child-process-watchdog.mjs", import.meta.url));
...
// maintenance prompts.
const AUTH_ADAPTER_PACKAGE_NAMES: ReadonlySet<string> = new Set(["pi-claude-auth", "@gotgenes/pi-anthropic-auth"]);

function isAuthAdapterPackageName(name: string): boolean {
  return AUTH_ADAPTER_PACKAGE_NAMES.has(name) || AUTH_ADAPTER_NAME_PATTERNS.some((pattern) => pattern.test(name));
}
