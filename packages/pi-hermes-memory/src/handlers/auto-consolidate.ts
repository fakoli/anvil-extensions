...
function entriesForTarget(store: MemoryStore, target: MemoryTarget): string[] {
  // Prefer raw entries (exact stored Markdown incl. metadata comments) so
  // consolidation round-trips preserve created/lastReferenced instead of
  // silently re-encoding fresh dates on every run — same contract as the
  // sync path in memory-tool.ts. Duck-typed: older/mock stores without the
  // raw variant fall back to the public getters.
  const raw = store.getRawEntriesForSync;
  if (typeof raw === "function") return raw.call(store, target);
...
function buildConsolidationPrompt(target: MemoryTarget, toolTarget: ToolMemoryTarget, entries: string[]): string {
  return [
    CONSOLIDATION_PROMPT,
    "",
