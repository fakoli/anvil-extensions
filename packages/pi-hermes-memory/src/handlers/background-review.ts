...
import { type DirectReviewResult, runDirectMemoryCompletion, usesDirectTransport } from "./review-memory-ops.js";
    ...
    const runReview = async (): Promise<void> => {
      ...
      if (usesDirectTransport(config)) {
        try {
          ...
          directFailure = [directResult.fallbackReason ?? "failed", directResult.error].filter(Boolean).join(": ");
        } catch (error) {
      ...
      if (subprocessResult.code === 0) {
        ...
        const subprocessDetail =
          subprocessResult.stderr?.trim() || subprocessResult.stdout?.trim() || `exit code ${subprocessResult.code}`;
        ctx.ui.notify(
          `Memory auto-review failed in both transports. Direct: ${diagnosticDetail(directFailure)}. ` +
