import { type ToolRenderResultOptions, keyHint } from "@earendil-works/pi-coding-agent";
import { type Component, Text, sliceByColumn, truncateToWidth, visibleWidth } from "@earendil-works/pi-tui";
...
function themed(theme: SharedOutputTheme | undefined, status: SharedStatus, partial: boolean, text: string): string {
...
function restoreBackground(text: string, background: ToolCardBackground, theme: SharedOutputTheme | undefined): string {
...
function renderView(
  ...
  if (options.expanded) {
    return new Text(view.expandedText || view.summary, 0, 0, (line) => restoreBackground(line, background, theme));
  }
  ...
  return {
    render(width: number): string[] {
      ...
      const partialPrefix =
        options.isPartial && !/progress|partial|in progress|处理中/i.test(view.summary) ? "In progress: " : "";
