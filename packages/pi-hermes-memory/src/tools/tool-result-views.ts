...
export function memoryResultView(result: unknown): SharedOutputView {
  ...
  const evicted =
    typeof data.evicted_count === "number"
      ? data.evicted_count
      : Array.isArray(data.evicted_entries)
        ? data.evicted_entries.length
        : 0;
  const outcome =
    /^Entry added\.$/.test(primaryMessage) || /^Failure memory saved:/.test(primaryMessage) || evicted > 0
      ? "Saved"
  ...
  if (typeof data.entry_count === "number")
    parts.push(`${data.entry_count} ${data.entry_count === 1 ? "entry" : "entries"}`);
