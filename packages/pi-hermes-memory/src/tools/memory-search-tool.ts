...
function mutationTarget(entry: { target: "memory" | "user" | "failure"; project: string | null }):
  | "memory"
  | "user"
  | "failure"
  | "project" {
    ...
    execute: async (
      ...
      if (stats.total === 0) {
        const result: SearchResult = {
          success: false,
          message: "No memories in extended store yet. Use memory_add to store memories.",
        };
        return { content: [{ type: "text" as const, text: result.message! }], details: result };
      }
