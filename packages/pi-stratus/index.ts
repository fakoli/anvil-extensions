// pi-stratus — cloud/network diagram engine for pi.
// Typed JSON spec → deterministic validation → reference-grade standalone
// HTML/SVG for AWS/GCP/Azure. Tools are thin wrappers over the engine; every
// tool returns structured data. render/validate route through
// compileAnyDiagram so beyond-cloud specs (workflow/sequence/dataflow/
// lifecycle) compile through the same entry point.


import { compileAnyDiagram, compileDiagram } from "./src/pipeline.ts";
import { getPreset, listPresets } from "./src/presets.ts";
import { runCli } from "./src/cli.ts";
import { exportDiagram, type ExportFormat } from "./src/export.ts";
import { evaluateDiagram } from "./src/evaluate.ts";
import { jevAssess, jevStatus } from "./src/jev.ts";
