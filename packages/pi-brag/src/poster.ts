...
//
// Process execution is injectable for offline tests.
...
export type RunFn = (args: string[]) => Promise<ExecOutcome>;
...
export const DEFAULT_RUN: RunFn = (args) =>
  new Promise((resolvePromise) => {
    execFile("ffmpeg", args, { encoding: "utf8", windowsHide: true }, (err, _stdout, stderr) => {
...
export async function makePoster(input: PosterInput, deps: PosterDeps = {}): Promise<PosterResult> {
  ...
  const extract = await run(buildExtractArgs(video, input.timestamp, poster));
  ...
  const bakeRun = await run(buildBakeArgs(video, poster, tmpOut));
  if (bakeRun.error) throw new Error(`ffmpeg unavailable: ${bakeRun.error}`);
  if (bakeRun.code !== 0) {
    return {
      poster,
      video,
