...
SFX live under `sfx/{casino,impact,interface,ui}/`, and the individual keypress set under `sfx/keyboard/`. The assets root is the pi-brag package's `skills/brag/assets/` directory — the `brag_doctor` tool prints the exact path. If the directory is empty, run the `brag_fetch_assets` tool once to download it from the upstream repo, or plan the video with `--no-music --no-sfx`.

Music lives at `<skill-assets>/music/`.

Bundled music cue presets live beside the music:

```text
<skill-assets>/music/cues/<track-stem>.music-cues.md
<skill-assets>/music/cues/<track-stem>.music-cues.json
```

SFX analysis lives beside the SFX library:

```text
<skill-assets>/sfx/sfx-analysis.md
<skill-assets>/sfx/sfx-analysis.json
```
...
cp <skill-assets>/sfx/interface/bong_001.ogg brag-output/composition/assets/sfx/interface/
cp <skill-assets>/sfx/impact/impactBell_heavy_000.ogg brag-output/composition/assets/sfx/impact/
cp <skill-assets>/music/happy-beats-business-moves-vol-1-by-ende-dot-app.mp3 brag-output/composition/assets/music/
```
...
2. **Any track → extended analysis (richest for custom tracks; needs Python, any Hyperframes version).** For a custom track — or to refresh a bundled one — run `analyze_music_cues.py` on the audio file. It produces the same rich cue JSON/Markdown for any track. The script is bundled with this skill at `<skill-dir>/scripts/` (next to this reference file). Run it via `uv`, which auto-provisions the deps (`librosa`, `numpy`, `scipy`, `soundfile`) from `<skill-dir>/scripts/pyproject.toml` — no manual `pip install` needed:
