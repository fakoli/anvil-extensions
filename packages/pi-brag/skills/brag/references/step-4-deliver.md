...
> **pi execution note.** Every command in this step can run as plain bash, but the
> `brag_render` tool is the preferred driver: it streams progress, enforces a
> timeout (120s for `check`/`snapshot`, 1200s for `render`), and survives long
> renders. `brag_poster` wraps the ffmpeg poster dance below (extract + frame-0
> bake) in one call. Raw commands are shown because they are the source of truth.
...
If the driving model cannot view images, delegate the snapshot review to a
vision-capable subagent, or skip it — the `check` gate already covers contrast
and layout overflow programmatically.
