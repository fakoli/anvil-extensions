...
When running as the pi-brag package, `<skill-assets>` is the package's `skills/brag/assets/` directory — the `brag_doctor` tool prints the exact path. If it is empty, run the `brag_fetch_assets` tool once (downloads from the upstream repo), or plan with `--no-music --no-sfx`.
