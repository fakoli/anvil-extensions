...
This skill ships inside the **pi-brag** extension package. The `/brag` command
pre-parses flags and hands you a kickoff prompt with the resolved options —
honor them. The package also registers tools that wrap the fiddly parts:

| Tool | Replaces | Notes |
|---|---|---|
| `brag_doctor` | ad-hoc env checks | node/ffmpeg/engine-CLI/asset status; run first if the environment is unverified |
| `brag_render` | raw `npx hyperframes` bash calls | streams progress, enforces timeouts (120s check / 1200s render), abort-safe |
| `brag_poster` | the ffmpeg poster dance | extract + frame-0 bake in one call |
| `brag_fetch_assets` | manual asset download | one-time fetch of bundled music/SFX from the upstream repo |

Bundled music/SFX live in this package's `skills/brag/assets/` (see
[references/audio.md](references/audio.md) for the layout). The assets are
**not vendored** to keep the package light: if the directory is empty, run
`brag_fetch_assets` once, or plan with `--no-music --no-sfx`.

If a visual gut-check is wanted and the driving model cannot view images,
delegate snapshot review to a vision-capable subagent, or rely on the
programmatic `check` gate (contrast + layout overflow).
...
When music is selected, include a compact `Music cue guidance` section: read the bundled track's cue preset from `<skill-assets>/music/cues/` if present, otherwise note cues will be detected at composition time (any track now supports beat sync — see `references/audio.md`). Cue metadata is optional timing guidance only: story, readability, pacing, and product clarity stay primary.
