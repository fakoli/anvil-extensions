...
- **ffmpeg + ffprobe** (render, poster, bake) — on PATH, or a static build in
  `~/.pi/agent/bin` or `~/.local/bin`: the render engine child and the
  ffmpeg/ffprobe steps append both dirs to their PATH, so installs living
  there need no shell configuration
...
- **Runs:** `npx hyperframes` (long-lived, killed by process group on
  timeout/abort), `ffmpeg`/`ffprobe`, `tar`. Nothing else. The engine child
  and the ffmpeg/ffprobe steps get a PATH extended with `~/.pi/agent/bin` and
  `~/.local/bin` (existing dirs only, appended after the system PATH — system
  resolution order is preserved), so ffmpeg installs in those locations are
  found without shell configuration; the `tar` exec in `brag_fetch_assets`
