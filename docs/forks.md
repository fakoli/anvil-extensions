...
- **pi-stratus** — original cloud/network diagram engine (contract-inspired by the public archify skill; no upstream code). TypeScript engine, deterministic pipeline, 8 progressive-disclosure presets, 8 pi tools + 2 commands + 1 skill. Runtime dependencies `pptxgenjs@^4.0.1`, `puppeteer-core@^25.12.0`, and `sharp@^0.35.4` are declared in the package manifest and recorded in the root lock; exports are offline (no credentials, no network); the JEV bridge is strictly advisory-only. Its `UPSTREAM.md` records the inspiration, dependency integrity, and local divergence.
